...
class alifzerocmsCloseRegisterWizard(models.TransientModel):
    _name = 'alifzerocms.close.register.wizard'
    ...
    merit_register_id = fields.Many2one('alifzerocms.merit.register','Merit Register',related='register_id.merit_register_id')
    ...
    def close_register(self):
        ...
        # for app in self.merit_register_id.merit_application_ids:
            ...
            today = date.today()
            receipt_type = self.env['alifzerocms.receipt.type'].search([
					('name','=','Admission Fee')])
            ...
            search_payment = self.env['alifzerocms.fee.payment']
            vals = {
                'date': today,
    ...
    def process_payment(self, admission_invoice,journal_id,fee_payment):
        for payment in self:
            ...
            if invoice:  # and invoice.amount_total <= payment.amount:
                ...
                # config_academic_semester = self.env['ir.config_parameter'].get_param('alifzerocms.current_academic_semester')
                # if config_academic_semester:
                # 	new_semester = self.env['alifzerocms.academic.semester'].browse(int(config_academic_semester))
                # else:
                ...
                if days > 0:
                    ...
                    if invoice.student_id.batch_id and invoice.student_id.batch_id.late_fee_max_fine:
                        ...
                        late_fee_max_fine = self.env['ir.config_parameter'].sudo().get_param(
                            'alifzerocms_fee.late_fee_max_fine') or 10000
                    ...
                    if invoice.student_id.batch_id and invoice.student_id.batch_id.late_fee_per_day_fine:
                        ...
                        late_fee_per_day_fine = self.env['ir.config_parameter'].sudo().get_param(
                            'alifzerocms_fee.late_fee_per_day_fine')
                    ...
                    if latefee > 0:
                        latefee_receipt_type = self.env['ir.config_parameter'].sudo().get_param(
                            'alifzerocms_registration.latefee_receipt_type')
                        if not latefee_receipt_type:
                            raise UserError('Please configure the LateFee Receipt Type in Global Settings')
                        else:
                            latefee_receipt_type_id = self.env['alifzerocms.receipt.type'].browse(
                        ...
                        ledger_id = self.env['alifzerocms.student.ledger'].create(ledger_data)
                ...
                search_credit = self.env['alifzerocms.student.ledger'].search(
                ...
                if refund_amount > 0:
                    ...
                    refund_id = self.env['alifzerocms.student.refund.details'].create(refund_data)
                ...
                for rec in security_heads:
                    ...
                    self.env['alifzerocms.student.refund.details'].create(refund_data)
