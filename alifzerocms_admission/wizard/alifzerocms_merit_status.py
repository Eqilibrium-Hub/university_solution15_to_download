...
class alifzerocmsMeritStatus(models.TransientModel):
    _name = 'alifzerocms.merit.status'
    _description = 'Merit Status'

    @api.model
    def _get_applicant(self):
        applicant_id = self.env['alifzerocms.application.merit'].browse(self._context.get('active_id', False))
        if applicant_id:
            return applicant_id.id
        return True
    
    merit_id = fields.Many2one('alifzerocms.application.merit', string='Applicant',default=_get_applicant)
    application_id = fields.Many2one('alifzerocms.application','Application',related='merit_id.application_id')
    amount = fields.Float('Amount',related='merit_id.amount')
    merit_register_id = fields.Many2one('alifzerocms.merit.register','Merit Register', related='merit_id.merit_register_id' )
    ...
    def onchange_state(self):
        ...
        if self.state == 'done' and self.amount > 0:
            if not self.env['alifzerocms.admission.confirm.fee'].search([
    ...
    def change_status(self):
        fee_rec = self.env['alifzerocms.admission.confirm.fee'].search([
            ('application_id', '=', self.application_id.id), ('merit_id', '=', False), ('amount', '=', self.amount)])
        fee_rec.write({
            'state': 'done',
