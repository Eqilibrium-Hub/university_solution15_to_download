...
class StudentMeritListReport(models.AbstractModel):
    _name = 'report.alifzerocms_admission.student_merit_list_report'
    ...
    def _get_report_values(self, docsid, data=None):
        if data.get('form', False):
            ...
            merit_register = self.env['alifzerocms.merit.register'].browse(merit_register_id)
        elif docsid:
            merit_register = self.env['alifzerocms.merit.register'].browse(docsid[0])
        ...
        report = self.env['ir.actions.report']._get_report_from_name('alifzerocms_admission.student_merit_list_report')
