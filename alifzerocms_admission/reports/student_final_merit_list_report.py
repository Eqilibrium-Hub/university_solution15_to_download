...
class StudentFinalMeritListReport(models.AbstractModel):
    _name = 'report.alifzerocms_admission.student_final_merit_list_report'
    ...
    def _get_report_values(self, docsid, data=None):
        if data.get('form', False):
            ...
            register = self.env['alifzerocms.admission.register'].browse(register_id)
        elif docsid:
            register = self.env['alifzerocms.admission.register'].browse(docsid[0])
        ...
        report = self.env['ir.actions.report']._get_report_from_name('alifzerocms_admission.student_final_merit_list_report')
        application_docs = self.env['alifzerocms.application.merit']
        docargs = {
            'register': register,
