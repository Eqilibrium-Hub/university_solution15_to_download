...
class NonSubsidizedFormReport(models.AbstractModel):
    _name = 'report.alifzerocms_admission.non_subsidized_form_report'
    ...
    def _get_report_values(self, docsid, data=None):
        ...
        if  application_id:
            ...
            application = self.env['alifzerocms.application'].search([('id','=',application_id)])
            acadmics = self.env['alifzerocms.application.academic'].search([('application_id','=',application.id)],order='id')
            pref_ids = self.env['alifzerocms.application.preference'].search([('application_id','=',application.id)], order='id')
        ...
        report = self.env['ir.actions.report']._get_report_from_name('alifzerocms_admission.non_subsidized_form_report')
        application_docs = self.env['alifzerocms.application']
        docargs = {
            'doc_ids': [],
