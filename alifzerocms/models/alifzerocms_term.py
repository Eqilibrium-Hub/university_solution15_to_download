...
class alifzerocmsAcademicSession(models.Model):
    _name = 'alifzerocms.academic.session'
...
class alifzerocmsAcademicTerm(models.Model):
    _name = 'alifzerocms.academic.term'
    ...
    term_lines = fields.One2many('alifzerocms.academic.term.line', 'term_id', string='Term Lines',copy=True)
...
class alifzerocmsAcademicTermLine(models.Model):
    _name = 'alifzerocms.academic.term.line'
    ...
    term_id = fields.Many2one('alifzerocms.academic.term', string='Term', required=True, help='Academic Term')
    name = fields.Char(string='Name', required=True,)
    # description = fields.Text(string='Description', help="Description about the Term")
    # type = fields.Selection([('regular', 'Regular'), ('summer', 'Summer'), ('special', 'Special')], string='Type',
    #                         default='regular')
    sequence = fields.Integer(string='Sequence', required=True, default=10)
    planning_ids = fields.One2many('alifzerocms.academic.term.planning', 'term_line_id', string='Plannings', copy=True )
    campus_ids = fields.Many2many('alifzerocms.campus', 'campus_term_rel',
        'term_line_id', 'campus_id', string='Campuses',copy=True)
    institute_ids = fields.Many2many('alifzerocms.institute', 'institute_term__rel',
        'term_line_id', 'institut_id', string='Institutes',copy=True)
    career_ids = fields.Many2many('alifzerocms.career', 'career_term_rel',
        'term_line_id', 'career_id', string='Careers',copy=True)
    program_ids = fields.Many2many('alifzerocms.program', 'program_term_rel',
        'term_line_id', 'program_id', string='Program',copy=True)
    batch_ids = fields.Many2many('alifzerocms.batch', 'batch_term_rel',
...
class alifzerocmsAcademicTermPlanning(models.Model):
    _name = 'alifzerocms.academic.term.planning'
    _description = 'Term Planning'
    _order = 'sequence desc'
    
    term_line_id = fields.Many2one('alifzerocms.academic.term.line', string='Term Line')
    ...
    # campus_ids = fields.Many2many('alifzerocms.campus', string='Campus')
...
class alifzerocmsSemester(models.Model):
    _name = "alifzerocms.semester"
