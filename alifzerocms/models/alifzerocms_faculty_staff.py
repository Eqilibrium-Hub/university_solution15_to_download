...
class alifzerocmsFacultyStaffTag(models.Model):
    _name = 'alifzerocms.faculty.staff.tag'
    _description = 'Faculty Staff Tag'

    name = fields.Char(string="Faculty Tag", required=True)
    color = fields.Integer(string='Color Index')
    faculty_staff_ids = fields.Many2many('alifzerocms.faculty.staff', string='Teachers')
...
class alifzerocmsFacultyStaffPosition(models.Model):
    _name = 'alifzerocms.faculty.staff.position'
...
class alifzerocmsAward(models.Model):
    _name = 'alifzerocms.award'
    _description = 'Faculty Honor/Awards'
    
    award_type = fields.Selection([
        ('award', 'AWARD'), ('honor', 'HONOR')], string='Honor/Award Type', required=True, default='award', track_visibility='onchange')
    
    nomination = fields.Selection([
        ('nust', 'NUST'),
        ('other', 'Other')], string='Nominated By', required=True, default='nust', track_visibility='onchange')
    
    status = fields.Selection([
        ('awarded', 'Awarded'),
        ('not awarded', 'Not Awarded')], string='Status', required=True, default='awarded', track_visibility='onchange')
    
    awarding_body = fields.Selection([
        ('nust', 'NUST'),
        ('other', 'Other')], string='Awarding Body', required=True, default='nust', track_visibility='onchange')
...
class alifzerocmsFacultyStaff(models.Model):
    _name = 'alifzerocms.faculty.staff'
    ...
    tag_ids = fields.Many2many('alifzerocms.faculty.staff.tag', 'faculty_staff_tag_rel', 'faculty_staff_id', 'tag_id',
                               string='Tag')
    subject_ids = fields.Many2many('alifzerocms.subject', string='Subject(s)', track_visibility='onchange')
    employee_id = fields.Many2one('hr.employee', 'Linked Employee', required=True, ondelete="cascade")
    marital_status = fields.Selection('Marital Status', related='employee_id.marital', readonly=True, store=True)
    ...
    work_location = fields.Char('University Mailing Address', related='employee_id.work_location_id.name', readonly=True, store=True)
    ...
    position_id = fields.Many2one('alifzerocms.faculty.staff.position', 'Position')

    award_ids = fields.One2many('alifzerocms.award','faculty_staff_id','Awards')
    publication_ids = fields.One2many('alifzerocms.publication', 'faculty_staff_id', 'Publications')
    language_ids = fields.Many2many('alifzerocms.language', string='Languages')
    extra_activity_ids = fields.One2many('alifzerocms.extra.activity', 'faculty_staff_id', 'Activites')
    ...
    def sync_unitime_odoo(self, instructors):
        for instructor in instructors:
            position = self.env['alifzerocms.faculty.staff.position'].search(
                [('code', '=', instructor['position']['reference'])])
            if not position:
                position = self.env['alifzerocms.faculty.staff.position'].create({
                    'name': instructor['position']['label'],
                    'code': instructor['position']['reference'],
                })
            dept = self.env['alifzerocms.department'].search([('code', '=', instructor['deptcode'])])
            data = {
            ...
            pattern = self.env['alifzerocms.faculty.staff'].search([('unitimeId', '=', instructor['unitimeId'])])
            if pattern:
                pattern.write(data)
            else:
                pattern = self.env['alifzerocms.faculty.staff'].create(data)
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/alifzerocms/static/xls/alifzerocms_faculty_staff.xls'
        }]
...
class WizardFacultyEmployee(models.TransientModel):
    ...
    def create_employee(self):
        for record in self:
            ...
            faculty = self.env['alifzerocms.faculty.staff'].browse(active_id)
