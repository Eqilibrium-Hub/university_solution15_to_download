...
class alifzerocmsProgramStream(models.Model):
    _name = 'alifzerocms.program.stream'
...
class alifzerocmsCourseTag(models.Model):
    _name = 'alifzerocms.course.tag'
...
class alifzerocmsCourse(models.Model):
    _name = 'alifzerocms.course'
    ...
    career_id = fields.Many2one('alifzerocms.career', 'Career')
    ...
    prereq_ids = fields.One2many('alifzerocms.course.prereq', 'course_id', string='PreRequisits', tracking=True)
    equivalent_ids = fields.One2many('alifzerocms.course.equivalent', 'course_id', string='Course Equivalent', tracking=True)
    component_lines = fields.One2many('alifzerocms.course.component', 'course_id', string='Course Components', tracking=True)
    credits = fields.Float('Credits',compute='_compute_credits',store=True)
    coreq_course = fields.Many2one('alifzerocms.course', 'Co-Requisite', tracking=True)
    tag_ids = fields.Many2many('alifzerocms.course.tag', 'course_tag_rel','course_id','tag_id','Tags', tracking=True)

    outline = fields.Html('Outline')
    suggested_books = fields.Text('Suggested Books')
    stream_id = fields.Many2one('alifzerocms.program.stream', 'Stream', tracking=True)
    major_course = fields.Boolean('Major Course', default=False)
    self_enrollment = fields.Boolean('Self Enrollment', default=True)
    #
    study_scheme_line_ids = fields.One2many('alifzerocms.study.scheme.line','course_id','Scheme Lines')
    #
    _sql_constraints = [
    ...
    def create(self, vals):
        if vals.get('code', False):
            course = self.env['alifzerocms.course'].search([('code','=',vals.get('code'))])
            if course:
    ...
    def write(self, vals):
        if vals.get('code', False):
            course = self.env['alifzerocms.course'].search([('id','!=',self.id),('code', '=', vals.get('code'))])
            if course:
                raise ValidationError('Course with same Code already exist!')
    ...
    #     #         'date': datetime.now(),
    #     #     }
    #     #     self.env['alifzerocms.student.history'].create(history_data)
    #     #
    #     # if vals.get('tag_ids'):
    #     #     to_be_removed = self.env['alifzerocms.student.tag']
    #     #     updated_tags  = self.env['alifzerocms.student.tag'].search([('id','in',vals.get('tag_ids')[0][2])])
    ...
    #     # res = super(alifzerocmsStudent, self).write(vals)
    #     #
    ...
    def get_import_templates(self):
        return [{
            ...
            'template': '/alifzerocms/static/xls/alifzerocms_course.xls'
        }]


class alifzerocmsCourseComponent(models.Model):
    _name = 'alifzerocms.course.component'
    _description = 'CMS Course Component'

    course_id = fields.Many2one('alifzerocms.course', ondelete='cascade')
...
class alifzerocmsCoursePreReq(models.Model):
    _name = 'alifzerocms.course.prereq'
    _description = 'CMS Course PreRequist'

    course_id = fields.Many2one('alifzerocms.course', string='Course')
    prereq_id = fields.Many2one('alifzerocms.course', string='PreRequist')
    effective_date = fields.Date(string='Effective Date', help='Effective Date of PreRequist')


class alifzerocmsCourseEquivalent(models.Model):
    _name = 'alifzerocms.course.equivalent'
    _description = 'CMS Course Equivalent'
    _rec_name = 'course_id'

    course_id = fields.Many2one('alifzerocms.course', string='Course')
    equivalent_id = fields.Many2one('alifzerocms.course', string='Equivalent')
    effective_date = fields.Date(string='Effective Date', help='Effective Date of Equivalent')

#
class alifzerocmsCourseHistory(models.Model):
    _name = 'alifzerocms.course.history'
    _description = 'Course History'
    _order = 'course_id'
    _rec_name = 'course_id'

    course_id = fields.Many2one('alifzerocms.course', 'Course', required=True)
