...
class alifzerocmsChangeStudentRegID(models.TransientModel):
	_name ='alifzerocms.student.reg.change'
	...
	def change_student_reg_no(self):
		for student in self.student_ids:
			if student.new_id_number in self.env['alifzerocms.student'].search([]).filtered(lambda s:s.id != student.id).mapped('id_number'):
