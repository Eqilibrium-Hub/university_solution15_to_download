...
class alifzerocmsCreateStudentUserWiz(models.TransientModel):
	_name ='alifzerocms.student.user.wiz'
...
class alifzerocmsCreateFacultyUser(models.TransientModel):
	_name ='alifzerocms.faculty.user.wiz'
	_description = 'Create Faculty User'

	@api.model
	def _get_faculty(self):
		if self.env.context.get('active_model', False) == 'alifzerocms.faculty.staff' and self.env.context.get('active_ids', False):
			return self.env.context['active_ids']

	faculty_ids = fields.Many2many('alifzerocms.faculty.staff', string='Faculty Staff',
