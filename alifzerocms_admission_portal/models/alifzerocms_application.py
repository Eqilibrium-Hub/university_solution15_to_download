...
class alifzerocmsAdmissionApplication(models.Model):
    _inherit = 'alifzerocms.application'
    ...
    # domicile_id = fields.Many2one('alifzerocms.domicile', string='Domicile ID')
    ...
    guardian_income = fields.Char(string='Income')
    ...
    finance_ass_duration = fields.Selection([
        ('one','One Year'),('two','Two Years'),('three','Three Years'),('four','Four Years')],'Assistance Duration')
    finance_ass_amount = fields.Integer('Assistance Amount1')
    finance_ass_amt = fields.Selection([
        ('one', '100000-200000'), ('two', '200000-400000'), ('four', '400000-600000'), ('full', 'Full Tution Fee')], 'Assistance Amount')
    ...
    get_info_from = fields.Selection(
        [('University Website', 'University Website'), ('Newspapers', 'Newspapers'),('Social Media', 'Social Media'),
         ('NEC Awareness Sessions', 'NEC Awareness Sessions'), ('Leaflet at NTS Test Centres', 'Leaflet at NTS Test Centres'),
    ...
    step = fields.Selection(
        [('Basic Info', 'Basic Info'), ('Applying For Program', 'Applying For Program'),
         ('Invoice Downloaded', 'Invoice Downloaded'),('Invoice Detail Done', 'Invoice Detail Done'), ('Invoice Uploaded', 'Invoice Uploaded'),
         ('Voucher Verified', 'Vourcher Verified'), ('Voucher UnVerified', 'Vourcher UnVerified'),
         ('Academic Step Done', 'Academic Step Done'), ('nts', 'Test Detail'),
         ('Document Uploaded', 'Document Uploaded'), ('done', 'Done'), ],
        string='On Step', default='Basic Info', tracking=True)
    step_number = fields.Integer(string='Step Number', default=0)

    
    # For Entry Test Verification
    entry_test_status = fields.Selection([('Yes', 'Yes'), ('No', 'No'), ('Rejected', 'Rejected')], string='Verified?',
                                         default="No")
    # Inter_percentage
    inter_percentage = fields.Float('Intermediate Percentage', compute='_get_marks', digits=(8, 3),
                                           store=True)
    physics_percentage = fields.Float('Physics Percentage', compute='_get_marks', digits=(8, 3), store=True)
    chemistry_percentage = fields.Float('Chemistry/Computer Percentage', compute='_get_marks', digits=(8, 3), store=True)
    ...
    inter_specialization = fields.Selection([('p-che-m', 'Physics-Chemistry-Maths'), ('p-com-m', 'Physics-Computer-Maths'),('a-p-che-m', 'A-levels/Physics-Chemistry-Maths'), ('a-p-com-m', 'A-levels/Physics-Computer-Maths'), ('p-che-com-m', 'Physics-Chemistry-Computer-Maths'), ('p-che-adm', 'Physics-Chemistry-Add-Maths')
                                             , ('p-che-b', 'Physics-Chemistry-Biology'), ('dae-e', 'DAE-Electrical'), ('dae-m', 'DAE-Mechanical'), ('dae-c', 'DAE-Civil')],
    ...
    chemistry_aggregates = fields.Float('Chemistry/Computer Aggregate', compute='compute_aggregate', digits=(8, 3), store=True)
    ...
    def make_army_pre(self):
        for rec in self.search([('quota_id2', '!=', False), ('state', '=', 'confirm'), ('register_id', '=', 6)]):
            ...
            for pre in preference_ids:
                self.env['alifzerocms.application.preference'].create({'application_id': rec.id, 'program_id': pre.program_id.id, 'type': 'Armed Forces', 'preference': random.choice(pre_list)})
    ...
    @api.depends('academic_ids','academic_ids.obtained_marks', 'academic_ids.total_marks', 'academic_ids.degree_level', 'academic_ids.is_additional_maths')
    def _get_marks(self):
        for rec in self:
                ...
                if academic.degree_level == 'A-Level':  # SARFRAZ
                    ...
                    if academic.inter_result_status  == 'Complete':
                        if academic.a_level_che and academic.a_level_percentage >= 60:
                            ...
                            academic.application_id.chemistry_marks = per_list[grade_list.index(academic.a_level_che)] or 0
                            ...
                            academic.application_id.chemistry_marks = per_list[grade_list.index(academic.a_level_com)] or 0
    ...
    @api.depends('ssc_percentage', 'inter_percentage', 'register_id', 'register_id.matric_o_level_score', 'register_id.intermediate', 'register_id.entry_test', 'register_id.physics_marks', 'register_id.math_marks', 'register_id.chemistry_marks')
    def compute_aggregate(self):
        for rec in self:
            ...
            rec.total_aggregates = rec.ssc_aggregates + rec.inter_aggregates + rec.entry_aggregates + rec.physics_aggregates + rec.math_aggregates + rec.chemistry_aggregates
        ...
        for rec in self:
            if rec.fee_voucher_state == 'upload':
                rec.write({
                    ...
                    'step_number': rec.step_number + 1,
                })
                if (rec.is_dual_nationality):
                    template = self.env.ref('alifzerocms_admission_portal.mail_template_voucher_verified2')
                else:
                    template = self.env.ref('alifzerocms_admission_portal.mail_template_voucher_verified')
    ...
    def univerify_voucher(self):
        for rec in self:
            ...
            template = self.env.ref('alifzerocms_admission_portal.mail_template_voucher_un_verified')
            post_message = rec.message_post_with_template(template.id, composition_mode='comment')
    ...
    def _show_report(self, model, report_type, report_ref, download=False):
        ...
        if report_type == 'pdf' and download:
            ...
            if report_ref == 'alifzerocms_admission_portal.admission_final_report':
                self.write({
                    'fee_voucher_state': 'download',
                    'fee_voucher_download_date': date.today(),
                })
    ...
    def get_total_registration_list_view(self):
        return {'registration_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree1').id}

    @api.model
    def get_downloaded_list_view(self):
        return {'downloaded_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree2').id}

    @api.model
    def get_upload_list_view(self):
        return {'upload_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree3').id}

    @api.model
    def get_verified_list_view(self):
        return {'verified_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree4').id}

    @api.model
    def get_unverified_list_view(self):
        return {'unverified_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree5').id}

    @api.model
    def get_final_score_list_view(self):
        return {'final_score_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree6').id}

    @api.model
    def get_confirm_applications_list_view(self):
        return {'confirm_applications_list_id': self.env.ref('alifzerocms_admission_portal.view_alifzerocms_application_tree1').id}
...
class alifzerocmsAdmissionApplicationAcademic(models.Model):
    _inherit = 'alifzerocms.application.academic'
    ...
    def action_document_verified(self):
        ...
        for rec in application_id.academic_ids:
            if rec.document_status == 'Yes':
                count += 1
        if count >= 2 and application_id.entry_test_status == 'Yes':
            application_id.state = 'verified'


class alifzerocmsAdmissionApplicationDocuments(models.Model):
    _inherit = 'alifzerocms.application.documents'
...
class alifzerocmsAggregateConfig(models.Model):
    _name = 'alifzerocms.aggregate.config'
...
class alifzerocmsMeritRegister(models.Model):
    _inherit = 'alifzerocms.merit.register'

    state = fields.Selection([('draft', 'Draft'), ('open', 'Open'),('done', 'Done')], string='Status',default='draft', tracking=True)

    def make_open(self):
        if self.state == 'draft':
            self.state = 'open'
            for app_merit in self.merit_application_ids:
                template = self.env.ref('alifzerocms_admission_portal.mail_template_application_merit_list')
...
class alifzerocmsApplicationMerit(models.Model):
    _inherit = 'alifzerocms.application.merit'
