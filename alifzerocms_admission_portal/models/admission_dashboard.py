...
class Employee(models.Model):
    ...
    def get_total_applications(self):
        rec_count=0
        registers = self.env['alifzerocms.admission.register'].search([('state', '=', 'application')])
        if registers:
            rec_count = self.env['alifzerocms.application'].search_count([('register_id','in',registers.ids)])
        return rec_count
    ...
    def get_confirm_applications(self):
        ...
        registers = self.env['alifzerocms.admission.register'].search([('state', '=', 'application')])
        if registers:
            rec_count = self.env['alifzerocms.application'].search_count([('register_id', 'in', registers.ids),('state','=','confirm')])
        return rec_count
    ...
    def get_total_invoice_downloaded(self):
        ...
        registers = self.env['alifzerocms.admission.register'].search([('state', '=', 'application')])
        if registers:
            rec_count = self.env['alifzerocms.application'].search_count([('register_id', 'in', registers.ids),('fee_voucher_state','=','download')])
        return rec_count
    ...
    def get_total_invoice_uploaded(self):
        ...
        registers = self.env['alifzerocms.admission.register'].search([('state', '=', 'application')])
        if registers:
            rec_count = self.env['alifzerocms.application'].search_count(
    ...
    def get_total_academic_detail(self):
        rec_count = 0
        template = self.env.ref('alifzerocms_admission_portal.admission_education_detail')
        step = self.env['alifzerocms.application.steps'].sudo().search([('template', '=', template.id)])
    ...
    def get_total_document_uploaded(self):
        rec_count = 0
        template = self.env.ref('alifzerocms_admission_portal.admission_documents_upload')
        step = self.env['alifzerocms.application.steps'].sudo().search([('template', '=', template.id)])
