...
function downloadVoucher(state) {
    ...
    if(state== 'no' || state == 'download'){
        ...
        $("#step_text_msg").html('Only Image Uploaded');
    }
...
function save_voucher_details() {
    ...
    if (!(fee_voucher_state == 'upload0' || fee_voucher_state == 'upload')){
	   ...
	   odoo.define('alifzerocms_admission_portal.voucher_details', function (require) {
			"use strict";
...
function verifyETEAData(){
	var test_date = document.getElementById("test_date").value;
		...
		if (test_date != "" && applicantID != "" && test_score != "" && applicant_name != "") {
			odoo.define('alifzerocms_admission_portal.clickevent', function (require) {
				"use strict";
				var ajax = require('web.ajax');
				var core = require('web.core');
				var session = require('web.session');
...
function isSameAddress(){
	...
	var present_country_id = document.getElementById("present_country_id");
	...
	if(checkBox_is_same_address.checked){
			...
			present_country_id.value = document.getElementById("per_country").value;
			present_province.value = document.getElementById("per_province").value;
			...
			city.readOnly = false;
			present_country_id.readOnly = false;
			present_province.readOnly = false;
...
function compute_grades_total(){
    ...
    var grade_b_marks = 75;
    var grade_c_marks = 65;
    var grade_d_marks = 55;
    var grade_e_marks = 45;
    var grade_f_marks = 40;
    var grade_g_marks = 35;
    ...
    var total = (grade_aa * 100)+(grade_a * 100)+(grade_b * 100)+(grade_c * 100)+(grade_d * 100)+(grade_e * 100)+(grade_f * 100)+(grade_g * 100);
    var obtained = (grade_aa * 90)+(grade_a * 85)+(grade_b * 75)+(grade_c * 65)+(grade_d * 55)+(grade_e * 45)+(grade_f * 40)+(grade_g * 35);
...
function compute_grades_inter_total(){
    ...
    document.getElementById('a_level_total').value = total;
    document.getElementById('a_level_obtained').value = obtained;
    if (total>0){
