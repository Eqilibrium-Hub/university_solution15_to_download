...
class CustomerPortalMeritList(CustomerPortal):
	...
	def portal_applicant_merit_info(self, page=1, date_begin=None, date_end=None, sortby=None, **kw):
		...
		admission_reg = http.request.env['alifzerocms.admission.register'].sudo().search([('id','=',application.register_id.id)])

		applicant_merit = http.request.env['alifzerocms.application.merit'].sudo().search([
			('application_id', '=', application.id),
			('merit_register_id','=',admission_reg.merit_register_id.id)
		])

		locked_application = http.request.env['alifzerocms.application.merit'].sudo().search([
			('application_id', '=', application.id),
			('locked', '=', True)
		])

		cancelled_application = http.request.env['alifzerocms.application.merit'].sudo().search([
			('application_id', '=', application.id),
		...
		quota_ids = http.request.env['alifzerocms.admission.quota']
		...
		return request.render("alifzerocms_admission_portal.portal_applicant_merit", values)
	...
	def save_voucher(self, **kw):
		...
		applicant_merit_id = http.request.env['alifzerocms.application.merit'].sudo().search([('id','=',applicant_merit)])
		...
		if applicant_merit_id and applicant_merit_id.merit_register_id.date_end < date.today():
			...
			return request.render("alifzerocms_admission_portal.submission_message", values)
	...
	def download_admission_fee_voucher(self, **kw):
		...
		application = http.request.env['alifzerocms.application'].sudo().search([('cnic', '=', current_user.login)])
		return application._show_report(model=application, report_type='pdf', report_ref='alifzerocms_admission_portal.student_admission_invoice', download=True)
